...
lw_shared_ptr<partition_snapshot> memtable_entry::snapshot(memtable& mtbl, uint32_t version_to_read) {
    return _pe.read(mtbl.region(), _schema, partition_snapshot::default_phase, version_to_read);
}
/*
lw_shared_ptr<partition_snapshot> memtable_entry::moon_snapshot(memtable& mtbl, const query::partition_slice &slice) {
    return _pe.read_moon(mtbl.region(), _schema, slice);
}
*/
bool 
memtable::has_partition(const query::ring_position& pos) {
    managed_bytes::linearization_context_guard lcg;
    auto i = partitions.find(pos, memtable_entry::compare(_schema));
    return i == partitions.end() ? false : true;
}

/*
lw_shared_ptr<partition_snapshot>
memtable::read_by_moon(const dht::partition_range &range,
                       const query::partition_slice &slice) {
    const query::ring_position& pos = range.start()->value();
    auto snp = _read_section(*this, [&] () -> lw_shared_ptr<partition_snapshot> {
        managed_bytes::linearization_context_guard lcg;
        auto i = partitions.find(pos, memtable_entry::compare(_schema));
        return i->moon_snapshot(*this, slice);
    });
    // snp->version()->partition().moon_mp_read(s, range, slice, query_time, builder);
    return std::move(snp);
}
*/
...
memtable::make_flat_reader(schema_ptr s,
    ...
    if (query::is_single_partition(range)) {
        ...
        auto snp = _read_section(*this, [&] () -> lw_shared_ptr<partition_snapshot> {
            ...
            if (i != partitions.end()) {
                upgrade_entry(*i);
                return i->snapshot(*this, version_to_read);
            } else {
                return { };
            }
        });
...
void
memtable::apply(const frozen_mutation& m, const schema_ptr& m_schema, lw_shared_ptr<uint32_t> commit_version_ptr, db::rp_handle&& h) {
    with_allocator(allocator(), [this, &m, &m_schema, commit_version_ptr] {
        _allocating_section(*this, [&, this, commit_version_ptr] {
          with_linearized_managed_bytes([&, commit_version_ptr] {
            auto& p = find_or_create_partition_slow(m.key(*_schema));
            p.apply(*_schema, m.partition(), *m_schema, std::move(commit_version_ptr), m.get_version_to_write());
          });
        });
    });
    update(std::move(h));
