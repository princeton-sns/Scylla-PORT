...
mutation::data::data(dht::decorated_key&& key, schema_ptr&& schema, uint32_t version_to_write)
    : _schema(std::move(schema))
    , _dk(std::move(key))
    , _p(_schema)
    , version_to_write(version_to_write)
{ }

mutation::data::data(partition_key&& key_, schema_ptr&& schema, uint32_t version_to_write)
    : _schema(std::move(schema))
...
mutation::data::data(schema_ptr&& schema, dht::decorated_key&& key, const mutation_partition& mp, uint32_t version_to_write)
    : _schema(std::move(schema))
    , _dk(std::move(key))
    , _p(mp)
    , version_to_write(version_to_write)
...
mutation::data::data(schema_ptr&& schema, dht::decorated_key&& key, mutation_partition&& mp, uint32_t version_to_write)
    : _schema(std::move(schema))
    , _dk(std::move(key))
    , _p(std::move(mp))
    , version_to_write(version_to_write)
