...
uint32_t frozen_mutation::get_version_to_write() const {
    return mutation_view().version_to_write();
}
    ...
    : _pk(m.key())
{
    ...
    std::move(wom).write_table_id(m.schema()->id())
                  ...
                  .write_key(m.key())
                  .write_version_to_write(m.version_to_write())
                  .partition([&] (auto wr) {
