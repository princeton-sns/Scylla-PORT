...
public:
    ...
    atomic_cell_or_collection* find_cell(column_id id); // for moon
private:
...
public:
    ...
    void apply_reversibly(const column_definition& column, atomic_cell_or_collection& cell, uint32_t most_recent_version = 0);
    // See reversibly_mergeable.hh
    ...
    // See reversibly_mergeable.hh
    void apply_reversibly(const schema&, column_kind, row& src, uint32_t most_recent_version = 0);
    // See reversibly_mergeable.hh
...
public:
    ...
    // See reversibly_mergeable.hh
    void apply_reversibly(const schema& s, deletable_row& src, uint32_t most_recent_version = 0);
    // See reversibly_mergeable.hh
...
public:
    ...
    // See reversibly_mergeable.hh
    void apply_reversibly(const schema& s, rows_entry& e, uint32_t most_recent_version = 0) {
        _row.apply_reversibly(s, e._row, most_recent_version);
    }
    // See reversibly_mergeable.hh
...
public:
    ...
    void apply(const schema& s, mutation_partition&& p, uint32_t most_recent_version = 0);
    // Same guarantees and constraints as for apply(const schema&, const mutation_partition&, const schema&).
    void apply(const schema& this_schema, mutation_partition_view p, const schema& p_schema, uint32_t most_recent_version = 0);
