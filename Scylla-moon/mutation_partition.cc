...
auto apply_reversibly_intrusive_set(const schema& s, mutation_partition::rows_type& dst, mutation_partition::rows_type& src, uint32_t most_recent_version = 0) {
    ...
    try {
        ...
        while (src_i != src.end()) {
            ...
            if (i == dst.end() || cmp(src_e, *i)) {
                ...
                i->apply_reversibly(s, src_e, most_recent_version);
            }
            ++src_i;
        }
...
void
mutation_partition::apply(const schema& s, mutation_partition&& p, uint32_t most_recent_version) {
    ...
    auto revert_rows = apply_reversibly_intrusive_set(s, _rows, p._rows, most_recent_version);
...
void
mutation_partition::apply(const schema& s, mutation_partition_view p, const schema& p_schema, uint32_t most_recent_version) {
    if (p_schema.version() == s.version()) {
        mutation_partition p2(*this, copy_comparators_only{});
        partition_builder b(s, p2);
        p.accept(s, b);
        apply(s, std::move(p2), most_recent_version);
    } else {
        mutation_partition p2(*this, copy_comparators_only{});
...
void write_cell(RowWriter& w, const query::partition_slice& slice, ::atomic_cell_view c, uint32_t most_recent_version = 0) {
    assert(c.is_live());
    auto wr = w.add().write();
    ...
    }().write_value(c.value()).write_most_recent_version(most_recent_version); // haonan: write most_recent_versin field, and will send to client
        ...
        if (!cell) {
            ...
            if (def.is_atomic()) {
                ...
                if (!c.is_live()) {
                    ...
                    write_cell(writer, slice, cell->as_atomic_cell(), cell->most_recent_version);
                }
            } else {
...
void deletable_row::apply_reversibly(const schema& s, deletable_row& src, uint32_t most_recent_version) {
    _cells.apply_reversibly(s, column_kind::regular_column, src._cells, most_recent_version);
...
void
apply_reversibly(const column_definition& def, atomic_cell_or_collection& dst,  atomic_cell_or_collection& src, uint32_t most_recent_version = 0) {
    ...
    if (def.is_atomic()) {
        ...
        if (def.is_counter()) {
            ...
            if (compare_atomic_cell_for_merge(dst.as_atomic_cell(), src.as_atomic_cell()) < 0) {
                std::swap(dst, src);
                dst.most_recent_version = most_recent_version;
...
void
row::apply_reversibly(const column_definition& column, atomic_cell_or_collection& value, uint32_t most_recent_version) {
    static_assert(std::is_nothrow_move_constructible<atomic_cell_or_collection>::value
                  && std::is_nothrow_move_assignable<atomic_cell_or_collection>::value,
    ...
    if (_type == storage_type::vector && id < max_vector_size) {
        if (id >= _storage.vector.v.size()) {
            ...
            ::apply_reversibly(column, _storage.vector.v[id], value, most_recent_version);
        }
    } else {
        if (_type == storage_type::vector) {
            vector_to_set();
...
void row::apply_reversibly(const schema& s, column_kind kind, row& other, uint32_t most_recent_version) {
    if (other.empty()) {
        return;
    }
    if (other._type == storage_type::vector) {
    ...
    other.for_each_cell([&, most_recent_version] (column_id id, atomic_cell_or_collection& cell) {
        apply_reversibly(s.column_at(kind, id), cell, most_recent_version);
...
/*
future<>
moon_mp_read(schema_ptr s,
             lw_shared_ptr<partition_snapshot> moon_source,
             const dht::partition_range &range,
             const query::partition_slice &slice,
             query::result::builder &builder) {
    auto qrb = query_result_builder(*s, builder);
    qrb.consume_new_partition(range.start()->value().as_decorated_key());
    rows_entry re(clustering_key::make_empty());

    moon_source->version()->partition().clustered_row(*s, clustering_key::make_empty()).cells().for_each_cell_until([&] (column_id id, const atomic_cell_or_collection& cell) {
        if (id >= slice.partition_row_limit()) {
            return stop_iteration::yes;
        }
        // fetch cell using get_cell()
        re.row().cells().append_cell(id, cell.get_cell(slice.version_to_read));
        //re._row.cells().append_cell(id, cell.get_cell(slice.version_to_read));
        return stop_iteration::no;
    });
    clustering_row rr(re);
    qrb.consume(std::move(rr), row_tombstone(moon_source->version()->partition().partition_tombstone()), true);
    qrb.consume_end_of_partition();
    qrb.consume_end_of_stream();
