      ...
      try {
        ...
        Map<ByteBuffer,List<ColumnOrSuperColumn>> results =
            client.multiget_slice(keys, parent,
                predicate, readConsistencyLevel, 0);

        if (debug) {
          System.out.print("Reading keys: " + keys);
        }

        // now check 1st round consistency
        Column c;
        int keys_ts = 0;
        ...
        for (Map.Entry<ByteBuffer, List<ColumnOrSuperColumn>> entry : results.entrySet()) {
          ...
          for (ColumnOrSuperColumn oneresult : entry.getValue()) {
            c = oneresult.column;
            if (c.most_recent_version > cols_ts) {
              cols_ts = c.most_recent_version;
            }
          }
          if (cols_ts != keys_ts && keys_ts != 0) {
            // inconsistent
            inconsistent = true;
          }
          if (cols_ts > keys_ts) {
        ...
        if (inconsistent) {
          // do 2nd round, the readTS is the max of the TSs from the 1st round
          results = client.multiget_slice(keys, parent, predicate, readConsistencyLevel, keys_ts);
        }
  ...
  public Status insert(String table, List<String> key_strs,
    ...
    for (int i = 0; i < operationRetries; i++) {
      ...
      try {
        ...
        int commit_version = client.batch_mutate(record, writeConsistencyLevel, 0);
