...
public class CassandraCQLClient extends DB {
   ...
   */
  @Override
  public Status read(String table, List<String> keys, Set<String> fields,
                     HashMap<ByteBuffer, HashMap<String, ByteIterator>> result) {

    try {
      String key = keys.get(0);  // just to make this compile, we are not using cql interface in scylla-moon.
      Statement stmt;
      ...
      if (!rs.isExhausted()) {
        ...
        for (ColumnDefinitions.Definition def : cd) {
          ...
          if (val != null) {
            value.put(def.getName(), new ByteArrayByteIterator(val.array()));
            result.put(ByteBuffer.wrap(key.getBytes("UTF-8")), value);
          } else {
            value.put(def.getName(), null);
            result.put(ByteBuffer.wrap(key.getBytes("UTF-8")), value);          }
        }
      ...
      e.printStackTrace();
      System.out.println("Error reading key: " + keys.get(0));
      return Status.ERROR;
    }
  ...
  public Status insert(String table, List<String> key_strs, HashMap<String,ByteIterator> values, boolean isLoading) {
    ...
    try {
      ...
      // Add key
      insertStmt.value(YCSB_KEY, key_strs);
