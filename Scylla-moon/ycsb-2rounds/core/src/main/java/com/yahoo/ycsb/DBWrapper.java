...
public class DBWrapper extends DB
{
   ...
   * @param key_strs A list of the record key of the record to read.
                     ...
                     HashMap<ByteBuffer, HashMap<String, ByteIterator>> result)
  {
    try (final TraceScope span = _tracer.newScope(SCOPE_STRING_READ)) {
      long ist=_measurements.getIntendedtartTimeNs();
      long st = System.nanoTime();
      Status res=_db.read(table, key_strs, fields, result);
      long en=System.nanoTime();
      ...
      HashMap<String,ByteIterator> values)
  {
    try (final TraceScope span = _tracer.newScope(SCOPE_STRING_UPDATE)) {
      long ist=_measurements.getIntendedtartTimeNs();
      long st = System.nanoTime();
      Status res=_db.update(table,key_strs,values);
      long en=System.nanoTime();
      ...
      HashMap<String,ByteIterator> values, boolean isLoading)
  {
    try (final TraceScope span = _tracer.newScope(SCOPE_STRING_INSERT)) {
      long ist=_measurements.getIntendedtartTimeNs();
      long st = System.nanoTime();
      Status res=_db.insert(table,key_strs,values, isLoading);
      long en=System.nanoTime();
