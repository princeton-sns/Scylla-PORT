...
class result_atomic_cell_view {
    ...
    ttl_opt _ttl;
    bytes_view _value;
    uint32_t _most_recent_version;
public:
    result_atomic_cell_view(api::timestamp_type timestamp, expiry_opt expiry, ttl_opt ttl, bytes_view value, uint32_t most_recent_version)
        : _timestamp(timestamp), _expiry(expiry), _ttl(ttl), _value(value), _most_recent_version(most_recent_version) { }
    ...
    int most_recent_version() const {
        // return an int instead, implicitly converted,
        // because thrift only allows signed type?
        return _most_recent_version;
    }
};
...
public:
    ...
    public:
        ...
        std::experimental::optional<result_atomic_cell_view> next_atomic_cell() {
            ...
            ttl_opt ttl = v.ttl();
            _tmp_value = v.value();
            // haonan: fetch most_recent_version field
            uint32_t most_recent_version = v.most_recent_version();
            return {result_atomic_cell_view(timestamp, expiry, ttl, _tmp_value, most_recent_version)};
        }
