...
column_family::make_reader(schema_ptr s,
    ...
    for (auto&& mt : *_memtables) {
        readers.emplace_back(mt->make_flat_reader(s, range, slice, pc, trace_state, fwd, fwd_mr, version_to_read));
    }
...
column_family::query(schema_ptr s, const query::read_command& cmd, query::result_request request,
    ...
    return f.then([this, lc, s = std::move(s), &cmd, request, &partition_ranges, trace_state = std::move(trace_state)] (query::result_memory_accounter accounter) mutable {
        ...
        return do_until(std::bind(&query_state::done, &qs), [this, &qs, trace_state = std::move(trace_state)] {
            ...
            /*
            if (qs.cmd.slice.options.contains(query::partition_slice::option::is_moon)) {
                // Haonan: this is moon rotxn
                if (qs.remaining_rows() == 0 || qs.cmd.slice.partition_row_limit() == 0 || qs.remaining_partitions() == 0) {
                    return make_ready_future<>();
                }
                const query::ring_position& pos = range.start()->value();
                for (auto&& mt : *_memtables) {
                    if (mt->has_partition(pos)) {
                        return moon_mp_read(qs.schema, std::move(mt->read_by_moon(range, qs.cmd.slice)), range, qs.cmd.slice, qs.builder);
                        //return mt->read_by_moon(qs.schema, range, qs.cmd.slice, qs.cmd.timestamp, qs.builder);
                    }
                }
                return make_ready_future<>();
            } else {
                return data_query(qs.schema, as_mutation_source(most_recent_version), range, qs.cmd.slice, qs.remaining_rows(),
                                  qs.remaining_partitions(), qs.cmd.timestamp, qs.builder, trace_state);
            }
            */
            if (qs.cmd.slice.options.contains(query::partition_slice::option::is_moon)) {
                return data_query(qs.schema, as_mutation_source(qs.cmd.slice.version_to_read), range, qs.cmd.slice, qs.remaining_rows(),
                                  qs.remaining_partitions(), qs.cmd.timestamp, qs.builder, trace_state);
            } else {
...
mutation_source
column_family::as_mutation_source(uint32_t version_to_read) const {
    return mutation_source([this, version_to_read] (schema_ptr s,
                                   const dht::partition_range& range,
                                   const query::partition_slice& slice,
                                   const io_priority_class& pc,
                                   tracing::trace_state_ptr trace_state,
                                   streamed_mutation::forwarding fwd,
                                   mutation_reader::forwarding fwd_mr) {
        return this->make_reader(std::move(s), range, slice, version_to_read, pc, std::move(trace_state), fwd, fwd_mr);
    });
}
...
void
column_family::apply(const frozen_mutation& m, const schema_ptr& m_schema, db::rp_handle&& h, lw_shared_ptr<uint32_t> commit_version_ptr) {
    do_apply(std::move(h), m, m_schema, std::move(commit_version_ptr));
}
...
future<> database::apply_in_memory(const frozen_mutation& m, schema_ptr m_schema, db::rp_handle&& h, timeout_clock::time_point timeout, lw_shared_ptr<uint32_t> commit_version_ptr) {
    auto& cf = find_column_family(m.column_family_id());
    return cf.dirty_memory_region_group().run_when_memory_available([this, &m, m_schema = std::move(m_schema), h = std::move(h), ptr = std::move(commit_version_ptr)]() mutable {
        try {
            auto& cf = find_column_family(m.column_family_id());
            cf.apply(m, m_schema, std::move(h), std::move(ptr));
        } catch (no_such_column_family&) {
...
future<> database::apply_with_commitlog(schema_ptr s, column_family& cf, utils::UUID uuid, const frozen_mutation& m, timeout_clock::time_point timeout, lw_shared_ptr<uint32_t> commit_version_ptr) {
    auto cl = cf.commitlog();
    if (cl != nullptr) {
        commitlog_entry_writer cew(s, m);
        return cf.commitlog()->add_entry(uuid, cew, timeout).then([&m, this, s, timeout, cl, ptr = std::move(commit_version_ptr)](db::rp_handle h) {
            return this->apply_in_memory(m, s, std::move(h), timeout, std::move(ptr)).handle_exception(maybe_handle_reorder);
        });
    }
    return apply_in_memory(m, std::move(s), {}, timeout, std::move(commit_version_ptr));
}

future<> database::do_apply(schema_ptr s, const frozen_mutation& m, timeout_clock::time_point timeout, lw_shared_ptr<uint32_t> commit_version_ptr) {
    ...
    if (cf.views().empty()) {
        return apply_with_commitlog(std::move(s), cf, std::move(uuid), m, timeout, std::move(commit_version_ptr));
    }
    auto f = cf.push_view_replica_updates(s, m);
    return f.then([this, s = std::move(s), uuid = std::move(uuid), &m, timeout, ptr = std::move(commit_version_ptr)] {
        auto& cf = find_column_family(uuid);
        return apply_with_commitlog(std::move(s), cf, std::move(uuid), m, timeout, std::move(ptr));
    });
}
...
future<> database::apply(schema_ptr s, const frozen_mutation& m, timeout_clock::time_point timeout, lw_shared_ptr<uint32_t> commit_version_ptr) {
    if (dblog.is_enabled(logging::log_level::trace)) {
        dblog.trace("apply {}", m.pretty_printer(s));
    }
    return update_write_metrics(apply_stage(this, std::move(s), seastar::cref(m), timeout, std::move(commit_version_ptr)));
}
