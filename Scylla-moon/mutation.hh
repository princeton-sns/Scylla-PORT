...
private:
    struct data {
        ...
        uint32_t version_to_write;

        data(dht::decorated_key&& key, schema_ptr&& schema, uint32_t version_to_write = 0);
        data(partition_key&& key, schema_ptr&& schema, uint32_t version_to_write = 0);
        data(schema_ptr&& schema, dht::decorated_key&& key, const mutation_partition& mp, uint32_t version_to_write = 0);
        data(schema_ptr&& schema, dht::decorated_key&& key, mutation_partition&& mp, uint32_t version_to_write = 0);
    };
...
public:
    mutation(dht::decorated_key key, schema_ptr schema, uint32_t version_to_write = 0)
        : _ptr(std::make_unique<data>(std::move(key), std::move(schema), version_to_write))
    { }
    mutation(partition_key key_, schema_ptr schema, uint32_t version_to_write = 0)
        : _ptr(std::make_unique<data>(std::move(key_), std::move(schema), version_to_write))
    { }
    mutation(schema_ptr schema, dht::decorated_key key, const mutation_partition& mp, uint32_t version_to_write = 0)
        : _ptr(std::make_unique<data>(std::move(schema), std::move(key), mp, version_to_write))
    { }
    mutation(schema_ptr schema, dht::decorated_key key, mutation_partition&& mp, uint32_t version_to_write = 0)
        : _ptr(std::make_unique<data>(std::move(schema), std::move(key), std::move(mp), version_to_write))
    { }
    mutation(const mutation& m)
        : _ptr(std::make_unique<data>(schema_ptr(m.schema()), dht::decorated_key(m.decorated_key()), m.partition(), m.version_to_write()))
    { }
    ...
    const uint32_t version_to_write() const { return _ptr->version_to_write; }
