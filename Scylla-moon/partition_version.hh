...
class partition_version : public anchorless_list_base_hook<partition_version> {
    ...
    uint32_t version_number = 0;    // for moon multiversion, the version identifier
    partition_snapshot* _snapshot = nullptr; // now each pv can have its snapshot
...
public:
    ...
    void set_version_number(uint32_t number) { this->version_number = number; }
    uint32_t get_version_number() { return this->version_number; }
};
...
public:
    ...
    explicit partition_version_ref(partition_version& pv) noexcept : _version(&pv), _evictable(false) { }
...
public:
    ...
    // constructor for per-pv snapshot
    explicit partition_snapshot(schema_ptr s,
                                logalloc::region& region,
                                partition_version& version,
                                phase_type phase = default_phase)
        : _schema(std::move(s)), _version(partition_version_ref(version)), _phase(phase), _region(region) { }
...
class partition_entry {
    ...
    uint32_t highest_version_read = 0;
    uint32_t highest_version_written = 0;
    uint32_t num_versions = 0;
    partition_version* last_version = nullptr;
...
public:
    ...
    void apply(const schema& s, mutation_partition_view mpv, const schema& mp_schema, lw_shared_ptr<uint32_t> commit_version_ptr = {}, uint32_t version_to_write = 0);

    /*
    void moon_read(schema_ptr s,
    ...
    */
    ...
    lw_shared_ptr<partition_snapshot> read(logalloc::region& region, schema_ptr entry_schema,
                                           ...
                                           uint32_t version_to_read = 0);

    /*
    lw_shared_ptr<partition_snapshot> read_moon(logalloc::region& region, schema_ptr entry_schema,
                                                const query::partition_slice &slice,
                                                partition_snapshot::phase_type phase = partition_snapshot::default_phase);
    */
