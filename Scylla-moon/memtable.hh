...
public:
    ...
    lw_shared_ptr<partition_snapshot> snapshot(memtable& mtbl, uint32_t version_to_read = 0);
    //lw_shared_ptr<partition_snapshot> moon_snapshot(memtable& mtbl, const query::partition_slice &slice);
...
public:
    ...
    // The mutation is upgraded to current schema.
    void apply(const frozen_mutation& m, const schema_ptr& m_schema, lw_shared_ptr<uint32_t> commit_version_ptr = {}, db::rp_handle&& = {});

    // Haonan: moon rotxn
    //lw_shared_ptr<partition_snapshot> read_by_moon(const dht::partition_range& range, const query::partition_slice& slice);

    bool has_partition(const query::ring_position& pos);
