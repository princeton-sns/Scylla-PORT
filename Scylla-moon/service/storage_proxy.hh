...
private:
    ...
    void got_response(response_id_type id, gms::inet_address from, uint32_t commit_version = 0);
    future<> response_wait(response_id_type id, clock_type::time_point timeout);
    future<uint32_t> moon_response_wait(storage_proxy::response_id_type id, clock_type::time_point timeout);
    ...
    future<uint32_t> moon_mutate_begin(std::vector<unique_response_handler> ids, db::consistency_level cl, stdx::optional<clock_type::time_point> timeout_opt = { });
    future<> mutate_end(future<> mutate_result, utils::latency_counter, tracing::trace_state_ptr trace_state);
    future<> moon_mutate_end(future<uint32_t> mutate_result, utils::latency_counter lc, tracing::trace_state_ptr trace_state, uint32_t& commit_version);
    ...
    template<typename Range>
    future<> moon_mutate_internal(uint32_t& commit_version, Range mutations, db::consistency_level cl, bool counter_write, tracing::trace_state_ptr tr_state, stdx::optional<clock_type::time_point> timeout_opt = { });
    ...
    future<int> do_moon_mutate(std::vector<mutation> mutations, db::consistency_level cl, tracing::trace_state_ptr tr_state, bool);
    ...
    friend class moon_mutate_executor;
public:
    ...
    // Applies mutation on this node.
    // Resolves with timed_out_error when timeout is reached.
    future<> mutate_locally(const schema_ptr&, const frozen_mutation& m, clock_type::time_point timeout = clock_type::time_point::max(), lw_shared_ptr<uint32_t> commit_version_ptr = {});
    ...
    */
    ...
    future<int> moon_mutate(std::vector<mutation> mutations, db::consistency_level cl, tracing::trace_state_ptr tr_state, bool raw_counters = false);
