...
protected:
    ...
    promise<uint32_t> _ready; // available when cl is achieved
...
public:
    ...
    virtual ~abstract_write_response_handler() {
        ...
        if (_cl_achieved) {
            if (_throttled) {
                _ready.set_value(commit_version);
            } else {
...
//            _ready.set_value(commit_version);
    ...
    future<> wait() {
        return _ready.get_future().discard_result();
    }
    future<uint32_t> moon_wait() {
    ...
    void set_commit_version(uint32_t v) {
        commit_version = v;
    }
...
void storage_proxy::got_response(storage_proxy::response_id_type id, gms::inet_address from, uint32_t commit_version) {
    auto it = _response_handlers.find(id);
    if (it != _response_handlers.end()) {
        tracing::trace(it->second.handler->get_trace_state(), "Got a response from /{}", from);
        it->second.handler->set_commit_version(commit_version);
    ...
    future<uint32_t> storage_proxy::moon_response_wait(storage_proxy::response_id_type id, clock_type::time_point timeout) {
        auto& e = _response_handlers.find(id)->second;

        e.expire_timer.arm(timeout);

        return e.handler->moon_wait();
    }
...
future<>
storage_proxy::mutate_locally(const schema_ptr& s, const frozen_mutation& m, clock_type::time_point timeout, lw_shared_ptr<uint32_t> commit_version_ptr) {
    auto shard = _db.local().shard_of(m);
    return _db.invoke_on(shard, [&m, gs = global_schema_ptr(s), timeout, p = std::move(commit_version_ptr)] (database& db) -> future<> {
        return db.apply(gs, m, timeout, std::move(p));
    });
}
...
future<std::vector<storage_proxy::unique_response_handler>> storage_proxy::mutate_prepare(const Range& mutations, db::consistency_level cl, db::write_type type, tracing::trace_state_ptr tr_state) {
    ...
    future<uint32_t> storage_proxy::moon_mutate_begin(std::vector<unique_response_handler> ids, db::consistency_level cl,
                                                      stdx::optional<clock_type::time_point> timeout_opt) {
        return moon_parallel_for_each(ids, [this, cl, timeout_opt] (unique_response_handler& protected_response) {
            ...
            auto f = moon_response_wait(response_id, timeout);
...
future<> storage_proxy::mutate_begin(std::vector<unique_response_handler> ids, db::consistency_level cl,
    ...
    future<> storage_proxy::moon_mutate_end(future<uint32_t> mutate_result, utils::latency_counter lc, tracing::trace_state_ptr trace_state, uint32_t& commit_version) {
        ...
        try {
            commit_version = mutate_result.get0();
            ...
            // timeout
            commit_version = 1; // TODO: need to make it retry or find a better way to pass back commit time
                                // TODO: okay for a prototype for now
...
static thread_local auto mutate_stage = seastar::make_execution_stage("storage_proxy_mutate", mutate_executor::get());
    ...
    struct moon_mutate_executor {
        static auto get() { return &storage_proxy::do_moon_mutate; }
    };
    static thread_local auto moon_mutate_stage = seastar::make_execution_stage("storage_proxy_moon_mutate", moon_mutate_executor::get());
...
future<> storage_proxy::mutate(std::vector<mutation> mutations, db::consistency_level cl, tracing::trace_state_ptr tr_state, bool raw_counters) {
    ...
    future<int> storage_proxy::moon_mutate(std::vector<mutation> mutations, db::consistency_level cl, tracing::trace_state_ptr tr_state, bool raw_counters) {
        return moon_mutate_stage(this, std::move(mutations), cl, std::move(tr_state), raw_counters);
    }

    future<int> storage_proxy::do_moon_mutate(std::vector<mutation> mutations, db::consistency_level cl, tracing::trace_state_ptr tr_state, bool raw_counters) {
        ...
        return seastar::when_all_succeed(
            ...
            moon_mutate_internal(commit_version, boost::make_iterator_range(mid, mutations.end()), cl, false, tr_state)
        ).then([&]{ return make_ready_future<int>(commit_version); });
    }
...
future<> storage_proxy::replicate_counter_from_leader(mutation m, db::consistency_level cl, tracing::trace_state_ptr tr_state,
    ...
    template<typename Range>
    future<>
    storage_proxy::moon_mutate_internal(uint32_t& commit_version, Range mutations, db::consistency_level cl, bool counters, tracing::trace_state_ptr tr_state,
                                   stdx::optional<clock_type::time_point> timeout_opt) {
        if (boost::empty(mutations)) {
            return make_ready_future<>();
        }
        ...
        return mutate_prepare(mutations, cl, type, tr_state).then([this, cl, timeout_opt, &commit_version] (std::vector<storage_proxy::unique_response_handler> ids) {
            return moon_mutate_begin(std::move(ids), cl, timeout_opt);
        }).then_wrapped([p = shared_from_this(), lc, tr_state, &commit_version] (future<uint32_t> f) mutable {
            return p->moon_mutate_end(std::move(f), lc, std::move(tr_state), commit_version);
        });
    }
...
void storage_proxy::send_to_live_endpoints(storage_proxy::response_id_type response_id, clock_type::time_point timeout)
{
    ...
    auto lmutate = [handler_ptr, response_id, this, my_address, timeout] (lw_shared_ptr<const frozen_mutation> m) mutable {
        ...
        lw_shared_ptr<uint32_t> commit_version_ptr = make_lw_shared<uint32_t>(1);
        return mutate_locally(std::move(s), *m, timeout, commit_version_ptr).then([pt = std::move(commit_version_ptr), response_id, this, my_address, m, h = std::move(handler_ptr), p = shared_from_this()] {
            // make mutation alive until it is processed locally, otherwise it
            // may disappear if write timeouts before this future is ready
            got_response(response_id, my_address, *pt);
        });
    };
...
void storage_proxy::init_messaging_service() {
    ...
    ms.register_mutation([] (const rpc::client_info& cinfo, rpc::opt_time_point t, frozen_mutation in, std::vector<gms::inet_address> forward, gms::inet_address reply_to, unsigned shard, storage_proxy::response_id_type response_id, rpc::optional<std::experimental::optional<tracing::trace_info>> trace_info) {
        ...
        return do_with(std::move(in), get_local_shared_storage_proxy(), [src_addr = std::move(src_addr), &cinfo, forward = std::move(forward), reply_to, shard, response_id, trace_state_ptr, timeout] (const frozen_mutation& m, shared_ptr<storage_proxy>& p) mutable {
            ...
            lw_shared_ptr<uint32_t> commit_version_ptr = make_lw_shared<uint32_t>();
            return when_all(
                // mutate_locally() may throw, putting it into apply() converts exception to a future.
                futurize<void>::apply([timeout, &p, &m, reply_to, src_addr = std::move(src_addr), commit_version_ptr] () mutable {
                    // FIXME: get_schema_for_write() doesn't timeout
                    return get_schema_for_write(m.schema_version(), std::move(src_addr)).then([&m, &p, timeout, commit_version_ptr] (schema_ptr s) {
                        return p->mutate_locally(std::move(s), m, timeout, commit_version_ptr);
                    });
                }).then([reply_to, shard, response_id, trace_state_ptr, ptr = std::move(commit_version_ptr)] () {
                    auto& ms = netw::get_local_messaging_service();
                    ...
                    return ms.send_mutation_done(netw::messaging_service::msg_addr{reply_to, shard}, shard, response_id, *ptr).then_wrapped([] (future<> f) {
    ...
    ms.register_mutation_done([] (const rpc::client_info& cinfo, unsigned shard, storage_proxy::response_id_type response_id, uint32_t commit_version) {
        auto& from = cinfo.retrieve_auxiliary<gms::inet_address>("baddr");
        return get_storage_proxy().invoke_on(shard, [from, response_id, commit_version] (storage_proxy& sp) {
            sp.got_response(response_id, from, commit_version);
            return netw::messaging_service::no_wait();
        });
    });
