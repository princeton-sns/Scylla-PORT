...
// haonan: parallel exec block variant for moon.
// could've made it a template, but I wanted it just for moon, so takes uint32_t value :)
struct moon_parallel_for_each_state {
    std::experimental::optional<std::exception_ptr> ex;
    size_t waiting = 0;
    promise<uint32_t> pr;
    uint32_t max_commit_version = 0;
    void complete(uint32_t value) {
        if (max_commit_version < value) {
            max_commit_version = value;
        }
        if (--waiting == 0) {
            if (ex) {
//                pr.set_value(max_commit_version);
...
GCC6_CONCEPT( requires requires (Func f, Iterator i) { { f(*i++) } -> future<uint32_t>; } )
inline
future<uint32_t>
moon_parallel_for_each(Iterator begin, Iterator end, Func&& func) {
    if (begin == end) {
        return make_ready_future<uint32_t>(0);
    }
    return do_with(moon_parallel_for_each_state(), [&] (moon_parallel_for_each_state& state) -> future<uint32_t> {
        // increase ref count to ensure all functions run
        ++state.waiting;
        while (begin != end) {
            ++state.waiting;
            try {
                func(*begin++).then_wrapped([&] (future<uint32_t> f) {
                    if (f.failed()) {
...
inline
future<uint32_t>
moon_parallel_for_each(Range&& range, Func&& func) {
    return moon_parallel_for_each(std::begin(range), std::end(range),
