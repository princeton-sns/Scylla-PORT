...
public:
    ...
    void get_slice(tcxx::function<void(std::vector<ColumnOrSuperColumn>  const& _return)> cob, tcxx::function<void(::apache::thrift::TDelayedException* _throw)> exn_cob, const std::string& key, const ColumnParent& column_parent, const SlicePredicate& predicate, const ConsistencyLevel::type consistency_level) {
        return multiget_slice([cob = std::move(cob)](auto&& results) {
            ...
            return cob({ });
        }, exn_cob, {key}, column_parent, predicate, consistency_level, 0);
    }
    ...
    void multiget_slice(tcxx::function<void(std::map<std::string, std::vector<ColumnOrSuperColumn> >  const& _return)> cob, tcxx::function<void(::apache::thrift::TDelayedException* _throw)> exn_cob, const std::vector<std::string> & keys, const ColumnParent& column_parent, const SlicePredicate& predicate, const ConsistencyLevel::type consistency_level, int32_t version_to_read) {
        with_schema(std::move(cob), std::move(exn_cob), column_parent.column_family, [&](schema_ptr schema) {
            ...
            assert(version_to_read >= 0);
            auto cmd = slice_pred_to_read_cmd(*schema, predicate, version_to_read);
    ...
    void batch_mutate(tcxx::function<void(int32_t const& _return)> cob, tcxx::function<void(::apache::thrift::TDelayedException* _throw)> exn_cob, const std::map<std::string, std::map<std::string, std::vector<Mutation> > > & mutation_map, const ConsistencyLevel::type consistency_level, int32_t version_to_write) {
        assert(version_to_write >= 0);
        with_cob(std::move(cob), std::move(exn_cob), [&] {
            auto p = prepare_mutations(_db.local(), current_keyspace(), mutation_map, version_to_write);
            return parallel_for_each(std::move(p.second), [this](auto&& schema) {
                ...
                return service::get_local_storage_proxy().moon_mutate(std::move(muts), cl_from_thrift(consistency_level), nullptr);
            });
        });
    }
...
private:
    ...
    static lw_shared_ptr<query::read_command> slice_pred_to_read_cmd(const schema& s, const SlicePredicate& predicate, int32_t version_to_read = 0) {
        auto opts = query_opts(s);
        std::vector<query::clustering_range> clustering_ranges;
        std::vector<column_id> regular_columns;
        uint32_t per_partition_row_limit = query::max_rows;
        //if (version_to_read > 0) {
        // this is a moon rotxn read, we assume all reads got from Thrift API are MOON rotxns for now.
        // this makes it easy for coding eiger-alike 2-roudn baseline
        opts.set(query::partition_slice::option::is_moon);
        //}
        ...
        auto slice = query::partition_slice(std::move(clustering_ranges), {}, std::move(regular_columns), opts,
                nullptr, cql_serialization_format::internal(), per_partition_row_limit, version_to_read);
    ...
    static Column make_column(const bytes& col, const query::result_atomic_cell_view& cell) {
        ...
        // return most_recent_version field to client over thrift
        ret.__set_most_recent_version(cell.most_recent_version());
        if (cell.ttl()) {
    ...
    static std::pair<std::vector<mutation>, std::vector<schema_ptr>> prepare_mutations(database& db, const sstring& ks_name, const mutation_map& m, uint32_t version_to_write = 0) {
        ...
        for (auto&& cf_key : m_by_cf) {
            ...
            for (auto&& key_mutations : cf_key.second) {
                mutation m_to_apply(key_from_thrift(*schema, to_bytes_view(key_mutations.first)), schema, version_to_write);
