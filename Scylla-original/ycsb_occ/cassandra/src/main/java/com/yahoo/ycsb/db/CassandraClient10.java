...
public class CassandraClient10 extends DB {
  ...
  public Status read(String table, List<String> key_strs, Set<String> fields,
    ...
    for (int i = 0; i < operationRetries; i++) {
      // get a buffer for returned results
      HashMap<ByteBuffer, HashMap<String, ByteIterator>> tmp_result = new HashMap<>();
      try {
        ...
        // this is the first round read, results put in result
        ...
        // now we do the second validation round
        Map<ByteBuffer,List<ColumnOrSuperColumn>> results2 =
                client.multiget_slice(keys, parent,
                        predicate, readConsistencyLevel);
        ...
        String name2;
        ByteIterator value2;
        for (Map.Entry<ByteBuffer, List<ColumnOrSuperColumn>> entry : results2.entrySet()) {
          tmp_result.put(entry.getKey(), new HashMap<String, ByteIterator>());
          for (ColumnOrSuperColumn oneresult : entry.getValue()) {
            column2 = oneresult.column;
            name2 = new String(column2.name.array(),
                    column2.name.position() + column2.name.arrayOffset(),
                    column2.name.remaining());
            value2 = new ByteArrayByteIterator(column2.value.array(),
                    column2.value.position() + column2.value.arrayOffset(),
                    column2.value.remaining());
            tmp_result.get(entry.getKey()).put(name2, value2);
            if (debug) {
              System.out.print("(" + name2 + "=" + value2 + ")");
            }
          }
        }
        // now comparing tmp_result and result for validation
        boolean success = true;
        assert tmp_result != null;
        for (Map.Entry<ByteBuffer, HashMap<String, ByteIterator>> entry : tmp_result.entrySet()) {
          ByteBuffer key = entry.getKey();
          for (Map.Entry<String, ByteIterator> col_val : entry.getValue().entrySet()) {
            String col = col_val.getKey();
            String read_2nd = ByteBufferUtil.bytesToHex(ByteBuffer.wrap(col_val.getValue().toArray()));
            String read_1st = ByteBufferUtil.bytesToHex(ByteBuffer.wrap(result.get(key).get(col).toArray()));
//            System.out.println("2nd read = " + read_2nd + "; 1st read = " + read_1st + "; they are NOT the same = " + !read_2nd.equals(read_1st));
            //if (!col_val.getValue().equals(result.get(key).get(col))) {
            if (!read_2nd.equals(read_1st)) {
              // if two returns are not the same then validation fails
//System.out.println("VALIDATION fails!");
              success = false;
              break;
            }
            /*
            if (!col_val.getValue().toString().equals(result.get(key).get(col).toString())) {
              // if two returns are not the same then validation fails
              success = false;
              break;
            }
