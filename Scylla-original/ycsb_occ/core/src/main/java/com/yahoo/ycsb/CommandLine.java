...
public class CommandLine
{
	 ...
	 for (;;)
	 {
	    ...
	    else if (tokens[0].compareTo("read")==0)
	    {
		  ...
		  HashMap<ByteBuffer, HashMap<String, ByteIterator>> result = new HashMap<>();
		  List<String> keys = new ArrayList<>();
		  keys.add(tokens[1]);
		  Status ret=db.read(table,keys,fields, result);
		  System.out.println("Return code: "+ret.getName());
		  for (Map.Entry<ByteBuffer, HashMap<String, ByteIterator>> ent : result.entrySet())
		  {
	    ...
	    else if (tokens[0].compareTo("update")==0)
	    {
		  ...
		  List<String> keys = new ArrayList<>();
		  keys.add(tokens[1]);
		  Status ret=db.update(table,keys,values);
		  System.out.println("Result: "+ret.getName());
	       }		  
	    }
	    else if (tokens[0].compareTo("insert")==0)
	    {
		  ...
		  List<String> keys = new ArrayList<>();
		  keys.add(tokens[1]);
		  Status ret=db.insert(table,keys,values, true);
		  System.out.println("Result: "+ret.getName());
	       }		  
	    }
